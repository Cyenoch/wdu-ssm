...
async function enrollOrgCA(
  ...
  console.info(`注册用户user0、orderer0、admin0 ${host}`)
  if (kind === 'peer') {
  ...
  console.info(`注册用户user0、orderer0、admin0 ${host} 注册成功`)

  if (kind === 'peer') {
    console.info(`生成 peer0 MSP ${host} 获取证书`)
    ...
    console.info(`生成 peer0 MSP ${host} 获取证书 成功`)


    console.info(`生成 user0 MSP ${host} 获取证书`)
    ...
    console.info(`生成 user0 MSP ${host} 获取证书 成功`)
  } else {
    console.info(`生成 orderer0 MSP ${host} 获取证书`)
    ...
    console.info(`生成 orderer0 MSP ${host} 获取证书 成功`)
